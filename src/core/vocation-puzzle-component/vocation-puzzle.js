...
function cardClicked(cardElement, cardData) {
  ...
  if (flippedCards.length === 2) {
    if (
      flippedCards[0].cardData.shared_id === flippedCards[1].cardData.shared_id
    ) {
      score++;
...
function showCompletionPopup() {
  const completionPopup = new bootstrap.Modal(
    document.getElementById("completion-popup")
  );
...
document
  .getElementById("submit-reflection")
